...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that LC3b uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
static bool CC_LC3b_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
static bool CC_LC3b_Assign_f64(unsigned &ValNo, MVT &ValVT,
                                MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
...
SDValue
LC3bTargetLowering::LowerReturn(SDValue Chain,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_LC3b32);
  ...
  if (MF.getFunction()->hasStructRetAttr()) {
    LC3bMachineFunctionInfo *SFI = MF.getInfo<LC3bMachineFunctionInfo>();
...
SDValue
LC3bTargetLowering::LowerFormalArguments(SDValue Chain,
                                          CallingConv::ID CallConv, bool isVarArg,
  ...
  LC3bMachineFunctionInfo *FuncInfo = MF.getInfo<LC3bMachineFunctionInfo>();

  // Assign locations to all of the incoming arguments.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, isVarArg, getTargetMachine(),
                 ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_LC3b32);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      // LC3b is big endian, so add an offset based on the ObjectVT.
...
SDValue
LC3bTargetLowering::LowerCall(SDValue Chain, SDValue Callee,
                               CallingConv::ID CallConv, bool isVarArg,
                               bool &isTailCall,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
  ...
  // LC3b target does not yet support tail call optimization.
  isTailCall = false;
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getTarget(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_LC3b32);
  ...
  for (unsigned i = 0, realArgIdx = 0, byvalArgIdx = 0, e = ArgLocs.size();
    ...
    if (VA.needsCustom()) {
      ...
      SDValue Store = DAG.getStore(DAG.getEntryNode(), dl,
                                   ...
                                   false, false, 0);
      // LC3b is big-endian, so the high part comes first.
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_LC3b32);
...
unsigned
LC3bTargetLowering::getSRetArgSize(SelectionDAG &DAG, SDValue Callee) const
{
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a LC3B ICC
/// condition.
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a LC3B
/// FCC condition.
...
LC3bTargetLowering::LC3bTargetLowering(TargetMachine &TM)
  ...
  // LC3b doesn't have i1 sign extending load
  ...
  // LC3b doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8 , Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1 , Expand);

  // LC3b has no REM or DIVREM operations.
  ...
  // LC3b has no select or setcc: expand to SELECT_CC.
  ...
  // LC3b doesn't have BRCOND either, it has BR_CC.
  ...
  // LC3B has no intrinsics for these particular operations.
  ...
  // FIXME: LC3b provides these multiplies, but we don't have them yet.
  ...
  if (TM.getSubtarget<LC3bSubtarget>().isV9())
    setOperationAction(ISD::CTPOP, MVT::i32, Legal);

  computeRegisterProperties();
}

const char *LC3bTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  default: return 0;
...
/// combiner.
void LC3bTargetLowering::computeMaskedBitsForTargetNode(const SDValue Op,
                                                         const APInt &Mask,
                                                         APInt &KnownZero,
                                                         APInt &KnownOne,
...
SDValue LC3bTargetLowering::LowerGlobalAddress(SDValue Op,
...
SDValue LC3bTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
...
LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  ...
  case ISD::GlobalTLSAddress:
    llvm_unreachable("TLS not implemented for LC3b.");
...
MachineBasicBlock *
LC3bTargetLowering::EmitInstrWithCustomInserter(MachineInstr *MI,
...
//===----------------------------------------------------------------------===//
//                         LC3b Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
LC3bTargetLowering::ConstraintType
LC3bTargetLowering::getConstraintType(const std::string &Constraint) const {
...
LC3bTargetLowering::getRegForInlineAsmConstraint(const std::string &Constraint,
                                                  EVT VT) const {
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
...
std::vector<unsigned> LC3bTargetLowering::
...
bool
LC3bTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The LC3b target isn't yet aware of offsets.
  return false;
}

/// getFunctionAlignment - Return the Log2 alignment of this function.
unsigned LC3bTargetLowering::getFunctionAlignment(const Function *) const {
  return 2;
}
