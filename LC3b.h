//===-- LC3b.h - Top-level interface for LC3b representation --*- C++ -*-===//
//
...
namespace llvm {
  ...
  FunctionPass *createLC3bISelDag(LC3bTargetMachine &TM);
  FunctionPass *createLC3bDelaySlotFillerPass(TargetMachine &TM);
  FunctionPass *createLC3bFPMoverPass(TargetMachine &TM);

  extern Target TheLC3bTarget;
  extern Target TheLC3bV9Target;

} // end namespace llvm;

// Defines symbolic names for LC3b registers.  This defines a mapping from
// register name to register number.
//
#include "LC3bGenRegisterNames.inc"

// Defines symbolic names for the LC3b instructions.
//
#include "LC3bGenInstrNames.inc"


namespace llvm {
  // Enums corresponding to LC3b condition codes, both icc's and fcc's.  These
  ...
  inline static const char *LC3BCondCodeToString(SPCC::CondCodes CC) {
    switch (CC) {
