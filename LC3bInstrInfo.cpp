//===- LC3bInstrInfo.cpp - LC3b Instruction Information -------*- C++ -*-===//
//
...
LC3bInstrInfo::LC3bInstrInfo(LC3bSubtarget &ST)
  : TargetInstrInfoImpl(LC3bInsts, array_lengthof(LC3bInsts)),
...
unsigned LC3bInstrInfo::isLoadFromStackSlot(const MachineInstr *MI,
...
unsigned LC3bInstrInfo::isStoreToStackSlot(const MachineInstr *MI,
                                            int &FrameIndex) const {
...
bool LC3bInstrInfo::AnalyzeBranch(MachineBasicBlock &MBB,
...
unsigned
LC3bInstrInfo::InsertBranch(MachineBasicBlock &MBB,MachineBasicBlock *TBB,
                             MachineBasicBlock *FBB,
                             const SmallVectorImpl<MachineOperand> &Cond,
                             DebugLoc DL) const {
  assert(TBB && "InsertBranch must not be told to insert a fallthrough");
  assert((Cond.size() == 1 || Cond.size() == 0) &&
         "LC3b branch conditions should have one component!");
...
unsigned LC3bInstrInfo::RemoveBranch(MachineBasicBlock &MBB) const
{
  MachineBasicBlock::iterator I = MBB.end();
  unsigned Count = 0;
  while (I != MBB.begin()) {
...
void LC3bInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
...
unsigned LC3bInstrInfo::getGlobalBaseReg(MachineFunction *MF) const
{
  LC3bMachineFunctionInfo *LC3bFI = MF->getInfo<LC3bMachineFunctionInfo>();
  unsigned GlobalBaseReg = LC3bFI->getGlobalBaseReg();
  if (GlobalBaseReg != 0)
  ...
  LC3bFI->setGlobalBaseReg(GlobalBaseReg);
  return GlobalBaseReg;
}
