//===-- LC3bAsmPrinter.cpp - LC3b LLVM assembly writer ------------------===//
//
...
namespace {
  class LC3bAsmPrinter : public AsmPrinter {
  public:
...
void LC3bAsmPrinter::printOperand(const MachineInstr *MI, int opNum,
                                   raw_ostream &O) {
...
void LC3bAsmPrinter::printMemOperand(const MachineInstr *MI, int opNum,
...
bool LC3bAsmPrinter::printGetPCX(const MachineInstr *MI, unsigned opNum,
                                  raw_ostream &O) {
  std::string operand = "";
  const MachineOperand &MO = MI->getOperand(opNum);
  switch (MO.getType()) {
...
void LC3bAsmPrinter::printCCOperand(const MachineInstr *MI, int opNum,
                                     raw_ostream &O) {
  int CC = (int)MI->getOperand(opNum).getImm();
  O << LC3BCondCodeToString((SPCC::CondCodes)CC);
}

/// PrintAsmOperand - Print out an operand for an inline asm expression.
///
bool LC3bAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
                                      unsigned AsmVariant,
...
bool LC3bAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
...
extern "C" void LLVMInitializeLC3bAsmPrinter() { 
  ...
  RegisterAsmPrinter<LC3bAsmPrinter> Y(TheLC3bV9Target);
}
